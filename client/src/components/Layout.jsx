...
import { NavLink, useNavigate, useLocation as useRouterLocation, Outlet } from 'react-router-dom';
...
export default function Layout() {
  ...
  // Fetch revenue for active location
  useEffect(() => {
    if (user?.role !== 'ADMIN') return;
    const params = new URLSearchParams();
    if (activeLocation?.id) params.set('venueIds', String(activeLocation.id));
    api.get(`/dashboard${params.toString() ? '?' + params.toString() : ''}`)
      .then(data => setRevenue(data.totalIncome || 0))
  ...
  return (
    <div className="app-layout">
      <header className="top-header">
        ...
        {/* Location Switcher */}
        <div className="location-switcher" ref={dropdownRef}>
          <button
            className="venue-selector"
            onClick={() => setLocationDropdownOpen(prev => !prev)}
          >
            {truncateName(activeLocation?.name || 'Select Location')} &#9662;
          </button>
          {locationDropdownOpen && (
            <div className="location-dropdown">
              <div className="location-dropdown-header">Switch Location</div>
              {locations.map(loc => (
                <button
                  key={loc.id}
              ...
              <button
                className="location-dropdown-item add-location"
                onClick={() => {
                  setLocationDropdownOpen(false);
                  navigate('/settings');
                }}
        ...
        <div className="header-right">
          <span className="revenue-badge">
            REVENUE GENERATED : ${revenue !== null ? ((revenue || 0) / 100).toFixed(2) : '...'}
          </span>
          <div className="header-avatar">{user?.name?.[0] || user?.firstName?.[0] || 'A'}</div>
        </div>
      </header>
